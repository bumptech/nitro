...
void trie_callback(const uint8_t *pfx, uint8_t length,
...
int main(int argc, char **argv) {
    ...
    td.num_matches = 0;
    nitro_prefix_trie_search(root,
    (const uint8_t *)"food", 4, trie_callback, &td);
    TEST("1 match sub", td.num_matches == 1);

    td.num_matches = 0;
    nitro_prefix_trie_search(root,
    (const uint8_t *)"fo", 2, trie_callback, &td);
    TEST("0 match super", td.num_matches == 0);

    td.num_matches = 0;
    nitro_prefix_trie_search(root,
    (const uint8_t *)"bright", 5, trie_callback, &td);
    ...
    td.num_matches = 0;
    nitro_prefix_trie_search(root,
    (const uint8_t *)"foodie", 6, trie_callback, &td);
    TEST("1 match on longer (still)", td.num_matches == 1);

    nitro_prefix_trie_add(&root,
        (const uint8_t *)"f", 1, &item_3);

    nitro_prefix_trie_add(&root,
        (const uint8_t *)"foodie", 6, &item_4);
    ...
    td.num_matches = 0;
    nitro_prefix_trie_search(root,
    (const uint8_t *)"tango", 5, trie_callback, &td);
    TEST("1 match on just wildcard",
        td.num_matches == 1); 

    r = nitro_prefix_trie_del(root,
        (const uint8_t *)"", 0, &item_1);
    ...
    td.num_matches = 0;
    nitro_prefix_trie_search(root,
    (const uint8_t *)"fem", 3, trie_callback, &td);
